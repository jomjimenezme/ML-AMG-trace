...
DEVEL_VERSION_FLAGS = $(CFLAGS) $(LIMEFLAGS) -DDEBUG -DPARAMOUTPUT -DTRACK_RES -DFGMRES_RESTEST -DPROFILING -DCOARSE_RES -DSCHWARZ_RES -DTESTVECTOR_ANALYSIS -DOPENMP -DPROFILING
...
$(BINDIR)/DDalphaAMG_devel : $(OBJDB)
	$(CC) -g $(DEVEL_VERSION_FLAGS) -o $@ $(OBJDB) $(H5LIB)  $(SCALAPACK_LIBRARIES) $(LAPACK_LIBRARIES) -lm -lgfortran
...
#$(CC) $(CFLAGS) -o $@ $@.c -I$(INCDIR) -I$(LAPACKE_INCLUDE) -L$(LIBDIR) -lDDalphaAMG $(SCALAPACK_LIBRARIES) $(LAPACK_LIBRARIES) -lm -lgfortran
